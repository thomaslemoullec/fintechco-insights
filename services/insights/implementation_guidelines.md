# Adding a new dashboard view — reuse map

Confirmed accurate as of 2026-07-14, maintained alongside the code it describes. Start
planning from these entries; open a specific file when you need its exact current syntax to
write against it.
...
Between this map and `DESIGN.md`, the reuse points for an additive dashboard view (backend and
frontend) are already documented here — no separate discovery pass is needed to relocate them.
Open the specific file named above directly when you need to confirm exact current syntax.
